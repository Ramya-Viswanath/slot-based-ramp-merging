#The code loads the model trained for various learning rates and then
#Experiments for 5 times and averages the results.
...
for lr in learning_rates:
    ...
    for exp in range(num_experiments):
        ...
        print(f"Experiment {exp + 1} - Learning Rate: {lr}")
    ...
    print(f"Average for Learning Rate {lr}")
    print("Successful merges ",all_successful_merges)
    print(all_unsuccessful_merges)
    ...
    results.append((lr, avg_successful_merges, avg_unsuccessful_merges, avg_rewards))
    plt.plot(avg_rewards, label=f'LR: {lr}', color=colors[j])
    j += 1

plt.xlabel('Episodes')
plt.ylabel('Average Rewards')
plt.title('Average Rewards for Different Learning Rates (Test Episodes)')
plt.legend()
plt.savefig("average_test_rewards.png")
plt.show()

for lr, avg_successful, avg_unsuccessful,_ in results:
    print(f"Overall Average for Learning Rate {lr}")
