...
class CustomActorCriticPolicy(ActorCriticPolicy):
    def __init__(self, observation_space, action_space, net_arch, *args, **kwargs):
        super(CustomActorCriticPolicy, self).__init__(observation_space, action_space, net_arch=net_arch, *args, **kwargs)
...
policies = ["CustomActorCriticPolicy64", "CustomActorCriticPolicy128"]

def train_and_save_models(policy):
    ...
    policy_kwargs = dict(
        net_arch=[64,64] if policy == "CustomActorCriticPolicy64" else [128,128],
    )
    ...
    model.save(f"ppo_merge_{policy}.zip")
    mean_reward, std_reward = evaluate_policy(model, env, n_eval_episodes=10)
    training_rewards = episodes_rewards.copy()
    np.save(f"training_rewards_{policy}.npy", training_rewards)
    return training_rewards, mean_reward, std_reward

results = []
for policy in policies:
    rewards, mean_reward, std_reward = train_and_save_models(policy)
    results.append((policy, rewards, mean_reward, std_reward))

results_df = pd.DataFrame(results, columns=["Policy", "Training Rewards", "Mean Reward", "Std Reward"])
results_df.to_csv("training_results_net_arch.csv", index=False)

results_df = pd.read_csv("training_results_net_arch.csv")

j=0
plt.figure(figsize=(15, 10))
for policy in policies:
    rewards = np.load(f"training_rewards_{policy}.npy")
    moving_avg_rewards = [np.mean(rewards[max(0, i-99):i+1]) for i in range(len(rewards))]
    label = policy.replace("CustomActorCriticPolicy", "Neurons: ")
    plt.plot(moving_avg_rewards, label=f'{label}', color=colors[j])
    j+=1
plt.xlabel('Episodes')
plt.ylabel('Rewards')
plt.title('Moving Average (last 100 episodes) vs Episodes for Different Actor Network Architectures')
plt.legend(title='Actor Network Neurons')
plt.savefig("training_rewards_different_net_archs.png")
plt.show()

test_episodes = 5000
test_results = []
for policy in policies:
    model = PPO.load(f"ppo_merge_{policy}.zip")
    ...
    print()
    test_results.append((policy, test_rewards.copy(), successful_test_episodes, unsuccessful_test_episodes))

test_results_df = pd.DataFrame(test_results, columns=["Policy", "Test Rewards", "Successful Merges", "Unsuccessful Merges"])
test_results_df.to_csv("test_results_net_arch.csv", index=False)

j=0
plt.figure(figsize=(15, 10))
for policy, test_rewards, _, _ in test_results:
    moving_avg_test_rewards = [np.mean(test_rewards[max(0, i-99):i+1]) for i in range(len(test_rewards))]
    label = policy.replace("CustomActorCriticPolicy", "Neurons: ")
    plt.plot(moving_avg_test_rewards, label=f'{label}', color = colors[j])
    j+=1
plt.xlabel('Episodes')
plt.ylabel('Rewards')
plt.title('Moving Average (last 100 episodes) vs Episodes for Different Actor Network Architectures')
plt.legend(title='Actor Network Neurons')
plt.savefig("test_rewards_comparison_net_archs.png")
plt.show()
