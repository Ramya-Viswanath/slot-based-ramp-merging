...
for gamma in discount_factors:
    ...
    for exp in range(num_experiments):
        ...
        print(f"Experiment {exp + 1} - Discount Factor: {gamma}")
    ...
    print(f"Average for Discount Factor {gamma}")
    ...
    results.append((gamma, avg_successful_merges, avg_unsuccessful_merges, avg_rewards))
    window_size = 100
    moving_avg_rewards = np.convolve(avg_rewards, np.ones(window_size) / window_size, mode='valid')
...
plt.title('Moving Average Rewards (last 100 episodes) for Different Discount Factors')
plt.legend()
plt.savefig("moving_average_test_rewards.png")
plt.show()

for gamma, avg_successful, avg_unsuccessful, _ in results:
    print(f"Overall Average for Discount Factor {gamma}")
